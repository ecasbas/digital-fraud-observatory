...
def validate():
    ...
    for c in CASES:
        ...
        assert c['kind'] in ('capture', 'reconstruction') and c['sources']
...
def card(c, prefix='./'):
    ...
    label = 'Actual website capture' if c['kind'] == 'capture' else 'Educational reconstruction'
    if c['kind'] == 'capture':
...
def case_page(c):
    prefix = '../../'
    kind = 'Actual website capture' if c['kind'] == 'capture' else 'Educational reconstruction'
